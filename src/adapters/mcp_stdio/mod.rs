...
impl McpServer {
    ...
    async fn handle_get_account_bank(
        &self,
        args: &serde_json::Value,
    ) -> Result<serde_json::Value, CallError> {
        let key = self.resolve_api_key(args)?;
    ...
    async fn handle_get_character_inventory(
        &self,
        args: &serde_json::Value,
    ) -> Result<serde_json::Value, CallError> {
        let raw_name = args
    ...
    async fn handle_get_market_prices(
        &self,
        args: &serde_json::Value,
    ) -> Result<serde_json::Value, CallError> {
        let ids = parse_required_id_array(args, "item_ids", ItemId::new)?;
        let v = self
            .service
            .get_market_prices(&ids)
            .await
            .map_err(|e| annotate_endpoint(e, "get_market_prices"))?;
        Ok(serde_json::to_value(&v)?)
    }
