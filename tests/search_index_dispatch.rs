...
use gw2_mcp::ports::{BuildCodeDecoder, Cache, CatalogRegistry, Clock, Gw2Api, SearchIndex, Wiki};
use gw2_mcp::service::Service;
use serde_json::{Value, json};

use crate::common::{FakeGw2Api, FakeWiki, TestCache, TestClock};
...
fn build_search_server(api: Arc<FakeGw2Api>, idx: Arc<dyn SearchIndex>) -> McpServer {
    ...
    let service = Service::new(gw2, wiki, cache, clock, decoder, catalogs).with_search_index(idx);
...
async fn search_disabled_returns_typed_error() {
    ...
    let decoder: Arc<dyn BuildCodeDecoder> = Arc::new(ChatrDecoder);
    let catalogs = Arc::new(CatalogRegistry::new());
    let service = Service::new(api, wiki, cache, clock, decoder, catalogs);
